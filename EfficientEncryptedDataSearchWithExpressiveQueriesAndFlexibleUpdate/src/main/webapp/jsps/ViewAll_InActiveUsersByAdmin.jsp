...
<title>Inactive users by admin</title>
...
<main>
  <div class="container py-5">
  	<div class="pt-4">
		...
		<div class="col-sm-10">
			<h4 class="mb-4">
				<strong><i>Inactive Data Users Details</i></strong>
			</h4>
				...
				<table class="table table-border table-hover">
					...
					<c:forEach items="${inActiveUsers}" var="inActiveUser">
						<tr>
							...
							<td>${inActiveUser.firstName}</td>
							<td>${inActiveUser.lastName}</td>
							<td>${inActiveUser.loginId}</td>
							<td>${inActiveUser.mobile}</td>
							<td class="d-none">${inActiveUser.status}</td>
							<td>In-Active</td>
							<td><form action="inActiveUsers" method="post">
								<input type="hidden" name="userId" value="${inActiveUser.userId}">
								<input type="hidden" name="status" value="1">
								<input class="btn btn-success" type="submit" value="Acept">
								</form>
							</td>
						</tr>
					</c:forEach>

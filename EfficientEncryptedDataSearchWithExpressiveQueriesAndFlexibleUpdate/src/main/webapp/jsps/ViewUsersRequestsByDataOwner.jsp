...
<title>view users requests by owner</title>
  	...
  	<div class="pt-4">
		...
		<div class="row pt-3">
			...
			<div class="col-sm-8">
				<table class="table table-border table-hover">
					...
					<c:forEach items="${requestsByAdmin}" var="record">
						<tr>
							<td>${record.rId}</td>
							<td>${record.fid}</td>
							<td class="d-none">${record.adminStaus}</td>
							<td>
								<c:if test="${record.adminStaus==0}">
									<div class="text-primary"><b>Accept</b></div>
								</c:if>
								<c:if test="${record.adminStaus==1}">
									<div class="text-danger"><b>Reject</b></div>
								</c:if>
							</td>
				...
				<div class="text-center text-info pb-3  text-danger">Node:- Admin can Accept/Reject the all requests !!</div>
